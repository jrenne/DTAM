...
#' A small numerical regularization is applied to the companion pencil to
#' handle singular augmented systems.
#'
...
solve_RE <- function(model) {
  ...
  # Build the companion pencil A s_t = B s_{t-1}. A small regularization is
  # needed for singular augmented representations used in the book.
  regularization <- 1e-8
  pencil_A <- rbind(
    cbind(identity, -Psi0),
    cbind(identity, matrix(0, n, n))
  ) + regularization * diag(2 * n)
  pencil_B <- rbind(
    cbind(Phi, Psi1),
    cbind(matrix(0, n, n), diag(n))
  ) + regularization * diag(2 * n)

  qz <- geigen::gqz(pencil_A, pencil_B, sort = "B")
  Z <- qz$Z

  Z_s <- matrix(Z[, seq_len(n)], 2 * n, n)

  ## Partition: Z_s = [ Z1 ; Z2 ]
  Z1 <- matrix(Z_s[seq_len(n), ], n, n)
  Z2 <- matrix(Z_s[n + seq_len(n), ], n, n)

  Phi_tilde <- Z2 %*% solve(Z1)

  Sigma12_tilde <- solve(identity - Psi0 %*% Phi_tilde) %*% Sigma12
  mu_tilde <- solve(identity - Psi1 - Psi0 %*% (identity + Phi_tilde)) %*% mu

  max_error <- max(Mod(Phi_tilde - Phi -
