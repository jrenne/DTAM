...
make_matrices_cond_mean_variance_Quadratic <- function(model, indic_compute_V){
  ...
  # Construct M_YZ and M_ZY
  # M_YZ is s.t. Y = M_YZ·Z
  # M_ZY is s.t. Z = M_YZ·Y,
  #    where Y = (x, vech(x·x')) and Z = (x, vec(x·x')).
  M_YZ <- matrix(0, n_total, n + n2)
  M_YZ[1:n, 1:n] <- Idn
  M_YZ[(n + 1):n_total, (n + 1):(n + n2)] <- M_vech_vec
  ...
  # Compute nu and Psi -------------------------------------------------------
  Jn_upper <- rbind(diag(n),
                    matrix(0,n^2,n))
  Jn_lower <- rbind(matrix(0,n,n^2),
                    diag(n^2))
  Pn_upper <- cbind( diag(n) %x% Jn_upper,   diag(n) %x% Jn_lower )
  Pn_lower <- cbind( diag(n^2) %x% Jn_upper, diag(n^2) %x% Jn_lower )
  Pi_n <- rbind(
    cbind(Pn_upper,matrix(0,dim(Pn_upper)[1],dim(Pn_lower)[2])),
    cbind(matrix(0,dim(Pn_lower)[1],dim(Pn_upper)[2]), Pn_lower))

  A12 <- ((Idn2 + Lambda_n) %x% Sigma) %*% (Idn %x% Lambda_n) %*%
    (vecX(Idn) %x% Idn)
  A21 <- (Sigma %x% (Idn2 + Lambda_n)) %*% (vecX(Idn) %x% Idn)
  A22 <- ((Idn2 + Lambda_n) %x% (Idn2 + Lambda_n)) %*%
    (Idn %x% (Lambda_n %x% Idn)) %*% (vecX(Sigma) %x% Idn2)
  C22 <- (Idn2 %x% (Idn2 + Lambda_n)) %*% vecX(Sigma %x% Sigma)

  nu <- Pi_n %*% rbind(vecX(Sigma),
                       A21 %*% mu,
                       A12 %*% mu,
                       A22 %*% (mu %x% mu) + C22)

  Psi <- Pi_n %*% rbind(matrix(0,n^2,n+n^2),
                        A21 %*% Phi1_tilde,
                        A12 %*% Phi1_tilde,
                        A22 %*% Phi2_tilde)
